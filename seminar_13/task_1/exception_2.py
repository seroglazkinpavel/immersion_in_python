...
from seminar_13.task_1.check import Check
from student_exception import StudentStrError, StudentLetterError,  StudentEstimateError, StudentSubjectsError
...
class Student:
    ...
    def data_storage(self, item_name: str, subject_grade: int, test_results: float):
        if item_name not in [self.subjects[i].get('subject') for i in range(len(self.subjects))]:
            raise StudentSubjectsError(item_name)

        if subject_grade < 2 or subject_grade > 5:
            raise StudentEstimateError(subject_grade, 2, 5)

        if test_results < 0 or test_results > 100:
            raise StudentEstimateError(test_results, 0, 100)
...
if __name__ == '__main__':
    student = Student('Petr', 'Sergeevich', 'Petrov', Path('subjects.csv'))
    #student = Student('9etr', 'ergeevich', 'etrov', Path('subjects.csv'))
    print(student.subjects)
    #student.data_storage('physics', 9, 50)
    #student.data_storage('physics', 2, 150)
    student.data_storage('physics', 5, 50)
